...
- **API returns 401:** check the CollectFolio Supabase issuer/JWKS values and confirm CollectFolio sends its current bearer token. This is separate from Cloudflare Tunnel authentication. If every token fails verification (not just an expired or malformed one), confirm CollectFolio's Supabase project uses asymmetric JWT signing keys (ES256/RS256) -- a legacy HS256 (symmetric-secret) project has no JWKS keys to discover and will fail all verification here regardless of how valid the token is. The service logs its own startup JWKS probe (key count/algorithms, or a warning naming this precondition) at boot; check those logs first.
